...
def generate_matrices_and_empty_result_file(rows_a, cols_a, rows_b, cols_b,
                                            value_range=(0, 100), output_dir="cmake-build-release"):
    if cols_a != rows_b:
    ...
    # create matrixA.txt, matrixB.txt and resultMatrix.txt

    with open(f"{output_dir}/matrixA.txt", 'w') as file:
        file.write(f"{rows_a} {cols_a}\n")
        np.savetxt(file, matrix_a, fmt="%d")

    with open(f"{output_dir}/matrixB.txt", 'w') as file:
        file.write(f"{rows_b} {cols_b}\n")
        np.savetxt(file, matrix_b, fmt="%d")

    open(f"{output_dir}/resultMatrix.txt", 'w').close()
